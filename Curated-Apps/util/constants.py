...
index = ['The target deployment environment is assumed to be an Azure Confidential compute'
         ...
         ' GSC image curation:', '1. Distro Selection', '2. Enclave Signing', '3. Attestation',
          '4. Secret Provisioning', '5. Command-line arguments', '6. Environment Variables',
           '7. Encrypted Files','8. Generation of the final curated image']
...
#todo                    ' DCAP client libs to verify'
                    ' the Quote supplied by the enclave. RA-TLS attestation flow requires you'
                    ...
                    ' RA-TLS flow. A test sample set of RA-TLS keys and'
...
arg_input = ['>> Runtime Arguments:', 'Specify docker command-line arguments here in a single string.'
             ' For example, if your docker runtime is ', 'docker run -it <image_name> arg1 arg2'
             + color_set, 'then the arguments that needs to be provided here are', 'arg1 arg2'
...
env_help =  ['This step secures the environment variables. Gramine will ignore environment'
             ...
             ' By default Gramine will add all the environment varialbes which the base docker'
             ' image already has.']
